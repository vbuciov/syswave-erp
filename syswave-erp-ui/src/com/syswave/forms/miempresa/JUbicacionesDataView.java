...
import com.syswave.forms.databinding.UbicacionTreeModel;
 ...
 */
public class JUbicacionesDataView extends JTreeDataView implements IUbicacionMediator, TreeModelListener
{
    ...
    UbicacionTreeModel bsUbicaciones;
    UbicacionSwingWorker swCargador;
    ...
    //---------------------------------------------------------------------
    public JUbicacionesDataView(IWindowContainer container)
    {
        super(container);
        initAttributes();
        grantAllPermisions();
    ...
    //---------------------------------------------------------------------
    public JUbicacionesDataView(IWindowContainer container, List<Usuario_tiene_Permiso> values)
    {
        super(container);
        initAttributes();
        grant(values);
    ...
    public final void grant(List<Usuario_tiene_Permiso> values)
    {
        ...
        //bsUbicaciones.setReadOnly(!can_update);
    }

    //---------------------------------------------------------------------
    private void initAttributes()
    {
        Ubicaciones = new UbicacionesBusinessLogic(mainContainer.getOrigenDatoActual().getNombre());
        bsUbicaciones = new UbicacionTreeModel();
        bsUbicaciones.setUseForCheckeBoxes(false);
        bsUbicaciones.setCanRecursiveAddToModified(false);
        bsUbicaciones.addTreeModelListener(this);
        jtData.setModel(bsUbicaciones);
        jtData.setShowsRootHandles(true);
        jtData.setRootVisible(false);
        jtData.setCellRenderer(new POJOTreeCellRenderer<Ubicacion>());
        jtData.setCellEditor(new POJOTreeCellEditor<Ubicacion>());
        jtData.setToggleClickCount(1); //Clicks Necesarios para expandir
        jtData.setEditable(true);
    }

    //---------------------------------------------------------------------
    public void showDetail(Ubicacion elemento)
    {
        JUbicacionesDetailView dialogo = new JUbicacionesDetailView(this);
        ...
        if (can_update)
        {
            if (jtData.getSelectionCount() > 0)
            {
                ...
                if (selected instanceof DefaultMutableTreeNode)
                    showDetail((Ubicacion) ((DefaultMutableTreeNode) selected).getUserObject());
            }
        }
        else
    ...
    public void doRetrieveProcess()
    {
        if (can_browse)
        {
            if (swCargador == null || swCargador.isDone())
            {
                ...
                jbMessage.setText("Consultado información...");
                swCargador.execute(parametros);
            }
        }
        else
    ...
    public void doSaveProcess()
    {
        ...
        List<Ubicacion> modificados = new ArrayList<>();
        List<Ubicacion> agregados = new ArrayList<>();
        List<Ubicacion> datos = bsUbicaciones.getModifiedDatas();

        for (Ubicacion elemento : datos)
        {
            if (elemento.isNew())
    ...
    //---------------------------------------------------------------------
    private void setDisplayData(List<Ubicacion> values)
    {
        bsUbicaciones.setData(values);

        jtData.expandPath(new TreePath(((DefaultMutableTreeNode) bsUbicaciones.getRoot()).getPath()));
    ...
    //---------------------------------------------------------------------
    private void setDisplayData(Ubicacion value, DefaultMutableTreeNode parentNode)
    {
        if (parentNode != null)
        {
            bsUbicaciones.addNodeTo(new DefaultMutableTreeNode(value), parentNode);
        }

        else
        {
            bsUbicaciones.addNodeToRoot(new DefaultMutableTreeNode(value));
        }
    ...
    //---------------------------------------------------------------------
    @Override
    public void onAcceptModifyElement(Ubicacion elemento)
    {
        if (swCargador == null || swCargador.isDone())
        {
            List<Object> parametros = new ArrayList<Object>();
    ...
    //---------------------------------------------------------------------
    @Override
    public void onAcceptNewElement(Ubicacion nuevo)
    {
        if (swCargador == null || swCargador.isDone())
        {
            ...
            if (jtData.getSelectionCount() > 0)
            {
                if (JOptionPane.showConfirmDialog(this, "¿Deseas agregar el nuevo elemento como hijo del elemento seleccionado?,\n en caso contrario será como una raíz", "Pregunta", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION)
                {
                    ...
                    Ubicacion parentElement = (Ubicacion) parentNode.getUserObject();
    ...
    //---------------------------------------------------------------------
    private void resetElement(Ubicacion value)
    {
        //value.acceptChanges();
        if (jtData.getSelectionCount() > 0)
        {
            TreeNode selected = (TreeNode) jtData.getSelectionPath().getLastPathComponent();
            bsUbicaciones.nodeChanged(selected);
        }
   ...
   /* private void removeNodes(TreePath[] nodes)
     ...
     bsUbicaciones.removeNodeFromParent((MutableTreeNode) selected);
     }
    ...
    //---------------------------------------------------------------------
    private class UbicacionSwingWorker extends SwingWorker<List<Object>, Void>
    {
        ...
        public List<Object> doInBackground()
        {
            ...
            if (!isCancelled() && arguments != null && arguments.size() > 0)
            {
                ...
                switch (opcion)
                {
                    ...
                    case opINSERT:
                        results.add(Ubicaciones.agregar((Ubicacion) arguments.get(1)));
                    ...
                    case opUPDATE:
                        results.add(Ubicaciones.actualizar((Ubicacion) arguments.get(1)));
                        /*arguments.add(UsuarioPermisos.guardar((Usuario)arguments.get(1), 
                         (List<NodoPermiso>)arguments.get(2), 
                         (int)arguments.get(3)));*/
                        break;
                    ...
                    case opDELETE_LIST:
                        List<Ubicacion> selecteds = (List<Ubicacion>) arguments.get(1);
                        results.add(Ubicaciones.borrar(selecteds));
                        break;

                    case opSAVE:
                        List<Ubicacion> adds = (List<Ubicacion>) arguments.get(1);
                        List<Ubicacion> modifieds = (List<Ubicacion>) arguments.get(2);
                        if (adds.size() > 0)
                            results.add(Ubicaciones.agregar(adds));
                        if (modifieds.size() > 0)
                            results.add(Ubicaciones.actualizar(modifieds));
                        break;
            ...
            try
            {
                if (!isCancelled())
                {
                    ...
                    if (results.size() > 0)
                    {
                        ...
                        {
                            case opLOAD:
                                List<Ubicacion> listaUsuarios = (List<Ubicacion>) results.get(1);
                                if (listaUsuarios.size() > 0)
                                    setDisplayData(listaUsuarios);
                                else if (!Ubicaciones.esCorrecto())
                                    JOptionPane.showMessageDialog(null, Ubicaciones.getMensaje());
                                break;

                            case opINSERT:
                                if (Ubicaciones.esCorrecto())
                                    setDisplayData((Ubicacion) arguments.get(1), (DefaultMutableTreeNode) arguments.get(2));

                                else
                                    jbMessage.setText(Ubicaciones.getMensaje());
                                break;

                            case opUPDATE:
                                if (Ubicaciones.esCorrecto())
                                    resetElement((Ubicacion) arguments.get(1));
                            ...
                            case opDELETE_LIST:
                                if (Ubicaciones.esCorrecto())
                                {
                                    bsUbicaciones.removeNodes((TreePath[]) arguments.get(2));
                                    printRecordCount();
                                    jbMessage.setText("Elemento(s) eliminado(s)");
                                }
                                else
