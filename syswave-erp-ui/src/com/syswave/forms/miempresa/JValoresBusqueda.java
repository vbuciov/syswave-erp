 ...
 */
public class JValoresBusqueda extends javax.swing.JInternalFrame implements PropertyChangeListener //TableModelListener
{
   private final int opLOAD_PRICE = 0;

   IValorBusquedaMediator owner;
   
   Valor limitado;
   ValoresBusinessLogic valores;
   ValorTableModel bsValores;

   ValoresBusquedaCargarSwingWorker swCargador;
   TableColumn colValor, colDescripcion, colActivo;
   /**
    * Creates new form JValoresBusqueda
    */
   public JValoresBusqueda(IValorBusquedaMediator parent)
   {
      owner = parent;
   ...
   {
      valores = new ValoresBusinessLogic(esquema);

      bsValores = new ValorTableModel(new String[]
      {
         "Valor:{valor}", "Descripción:{descripcion}", "Activo:{es_activo}"
      });
    ...
    */
   public void limitarSeccion(String limite)
   {
      Valor nuevo = new Valor ();
      nuevo.setSeccion(limite);
      limitado = nuevo;
   }
   ...
   public void busquedaInicial()
   {
      if (swCargador == null || swCargador.isDone())
      {
         ...
         swCargador = new ValoresBusquedaCargarSwingWorker();
   ...
   private void search_actionPerformed(ActionEvent evt)
   {
      if (swCargador == null || swCargador.isDone())
      {
         ...
         if (limitado == null)
            limitado = new Valor();
         limitado.setDescripcion(jtfValor.getText());
         parametros.add(limitado);
         showTaskHeader("Buscando, espere un momento...", true);
   ...
   //----------------------------------------------------------------------
   public void setDisplayData(List<Valor> listaPrecios)
   {
   ...
   //------------------------------------------------------------------
   private class ValoresBusquedaCargarSwingWorker extends SwingWorker<List<Object>, Void>
   {
            ...
            if (!isCancelled())
            {
               ...
               switch (opcion)
               {
                  case opLOAD_PRICE:
                     ...
                     {
                        setDisplayData((List<Valor>) results.get(2));
